...
export async function newsCommand(limit = 5) {
  try {
    logger.info('Fetching latest space news...');

    // Spaceflight News API - free, no authentication required
    const url = 'https://api.spaceflightnewsapi.net/v4/articles/?limit=' + limit;
    const response = await axios.get(url, { timeout: 10000 });

    // API v4 returns paginated results: { count, next, previous, results: [...] }
    ...
    logger.header(`Space News - Latest ${articles.length} Headlines`);

    articles.forEach((article, index) => {
      logger.log(`${(index + 1).toString().padStart(2, '0')}. ${article.title}`);
      logger.log(`    Source: ${article.news_site} | Date: ${new Date(article.published_at).toLocaleDateString()}`);
...
export async function quoteCommand() {
  try {
    ...
    if (quote.tags && quote.tags.length > 0) {
      logger.log(`Tags: ${quote.tags.join(', ')}`);
    }
...
export function registerApiCommands(program) {
  ...
  program
    .command('news [limit]')
    .description('Get latest space news headlines')
